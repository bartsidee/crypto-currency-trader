...
export let Sma = function(source: Candle[], period: number = 30, type: CandleVariable = CandleVariable.Close): (number| undefined)[] {
  ...
  return FixIndicatorOrdering(talib.SMA(
    valuesToCheck,
    period
  ), source.length);
};

export let SmaNumber = function(source: number[], period: number = 30): (number| undefined)[] {
  return FixIndicatorOrdering(talib.SMA(
    source,
    period
  ), source.length);
