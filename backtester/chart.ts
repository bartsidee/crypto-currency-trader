...
export let GenerateChart = function(market, strategy: ITradingStrategy, trades: Trade[]) {
  ...
  <script>
    ...
    var dim = {
        ...
        ohlc: { height: 410 },
        indicator: { height: 130, padding: 5 }
    };
    ...
    var data = JSON.parse('${JSON.stringify(strategy.Candles)}');
    ...
    var buys = JSON.parse('${JSON.stringify(trades.map((t) => {
      return { date: t.OpenDate, type: "buy", price: t.OpenRate, quantity: t.Quantity };
    }))}');

    var sells = JSON.parse('${JSON.stringify(trades.map((t) => {
      return { date: t.CloseDate, type: "sell", price: t.CloseRate, quantity: t.Quantity };
    }))}');

    var trads = JSON.parse('${JSON.stringify(trades)}');

    var trades = (buys.concat(sells)).map(function(x) {
...
`;
  fs.writeFileSync(path.join(__dirname, "charts", `results-${market}-${strategy.Name}.html`), data);
